...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	float belief = 1.0/(height * width);

	vector< vector <float> > newgrid(height, vector<float> (width, belief));
...
vector< vector <float> > move(int dy, int dx,
	...
	for (int i=0; i < height; i++) {
		for (int j=0; j < width; j++) {
			int i_prime = (i + dy + height) % height;
			int j_prime = (j + dx + width) % width;
			newGrid[i_prime][j_prime] = beliefs[i][j];
		}
	}
